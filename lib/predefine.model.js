...
const BUCKETS = _.map(NAMESPACES, namespace => collectionNameOf(namespace));
...
const PredefineSchema = new Schema({
  ...
  name: {
    ...
    taggable: true,
    exportable: true,
    fake: f => _.upperFirst(f.random.word())
  },
...
PredefineSchema.methods.preValidate = function preValidate(done) {
  ...
  // ensure namespace
  this.namespace = (_.trim(this.namespace) || DEFAULT_NAMESPACE);

  // ensure correct bucket
  const actualBucket = collectionNameOf(this.namespace);
  const derivedBucket = (_.trim(this.bucket) || DEFAULT_BUCKET);
  this.bucket = derivedBucket === actualBucket ? derivedBucket : actualBucket;
...
PredefineSchema.statics.prepareSeedCriteria = seed => {
  const criteria = (
    seed._id ?
    _.pick(seed, '_id') :
    _.pick(seed, 'namespace', 'bucket', 'name')
  );
  return criteria;
};
...
PredefineSchema.plugin(exportable);
