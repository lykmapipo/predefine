...
PredefineSchema.statics.findRecursive = (criteria, done) => {
  ...
  const collectResults = (updates) => {
    let collected = common.uniq([].concat(results).concat(updates));
    collected = lodash.uniqWith(collected, (a, b) => {
      return a.equals(b);
    });
    return collected;
  };
  ...
  const findAncestors = (next) => {
    return Predefine.find(criteria)
      ...
      .exec((error, ancestors) => {
        ...
        const parentIds = common.uniq(lodash.map(ancestors, '_id'));
  ...
  const findChildrenRecursive = (parents, next) => {
    ...
    if (!lodash.isEmpty(parents)) {
      return findChildren(parents, (error, children) => {
        ...
        const parentIds = common.uniq(lodash.map(children, '_id'));
