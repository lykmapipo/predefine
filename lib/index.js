...
const PredefineSchema = mongooseCommon.createSchema(
  {
     ...
     *
     */
    abbreviation: mongooseLocaleSchema.localize({
      type: String,
      trim: true,
      index: true,
      searchable: true,
...
PredefineSchema.methods.preValidate = function preValidate(done) {
  ...
  this.description = common.mergeObjects(
    mongooseLocaleSchema.localizedValuesFor(this.name),
    mongooseLocaleSchema.localizedValuesFor(this.description)
  );

  // ensure abbreviation for all locales
  this.abbreviation = common.mergeObjects(
    mongooseLocaleSchema.localizedAbbreviationsFor(this.name),
    mongooseLocaleSchema.localizedValuesFor(this.abbreviation)
  );
  ...
  const bucketAndNamespace = common.mergeObjects(
    env.isTest ? {} : { bucket: DEFAULT_BUCKET, namespace: DEFAULT_NAMESPACE },
