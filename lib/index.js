...
const COLLECTION_NAME = env.getString('PREDEFINE_COLLECTION_NAME', 'predefines');
const SCHEMA_OPTIONS = { collection: COLLECTION_NAME };
const DEFAULT_NAMESPACE = env.getString('PREDEFINE_DEFAULT_NAMESPACE', 'Setting');
const NAMESPACES = common.sortedUniq([
  DEFAULT_NAMESPACE,
  ...env.getStrings('PREDEFINE_NAMESPACES', DEFAULT_NAMESPACE),
]);
...
PredefineSchema.methods.preValidate = function preValidate(done) {
  ...
  _.forEach(LOCALES, locale => {
    ...
    abbreviation = _.trim(abbreviation) || common.abbreviate(_.get(this.name, locale));
