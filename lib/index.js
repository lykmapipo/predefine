...
const parseGivenRelations = () => {
  ...
  relations = lodash.mapValues(relations, relation => {
    ...
    return common.mergeObjects(relation, {
      type: array ? [mongooseCommon.ObjectId] : mongooseCommon.ObjectId,
      ref: ref || MODEL_NAME,
      index: true,
      autopopulate: common.mergeObjects(autopopulate, { maxDepth: 1 }),
      taggable: true,
      // exportable: true,
      aggregatable: { unwind: true },
      default: undefined,
...
const normalizeQueryFilter = (optns = {}) => {
  ...
  const isDefaultBucket = lodash.get(options, 'filter.bucket') === 'defaults';
  if (isDefaultBucket) {
...
const transformToPredefine = val => {
  ...
  const predefine = common.mergeObjects({
    strings: lodash.mapValues(lodash.pick(data, ...stringSchemaPaths()), (value, key) => {
      if (lodash.includes(LOCALIZED_STRING_PATHS, key)) {
        return mongooseLocaleSchema.localizedValuesFor({ en: value });
      }
      return value;
    }),
    numbers: lodash.pick(data, ...numberSchemaPaths()),
    booleans: lodash.pick(data, ...booleanSchemaPaths()),
    dates: lodash.pick(data, ...dateSchemaPaths()),
...
PredefineSchema.statics.prepareSeedCriteria = seed => {
  ...
  copyOfSeed.name = mongooseLocaleSchema.localizedValuesFor(seed.strings.name);

  const criteria = common.idOf(copyOfSeed)
    ? lodash.pick(copyOfSeed, '_id')
    : common.flat(lodash.pick(copyOfSeed, 'namespace', 'bucket', 'strings.code', ...names));
  return criteria;
};
...
PredefineSchema.statics.getByExtension = (optns, done) => {
  ...
  // return
  return async.waterfall([get, transform], done);
};
...
PredefineSchema.statics.postByExtension = (optns, done) => {
  ...
  // return
  return async.waterfall([post, transform], done);
};
...
PredefineSchema.statics.getByIdByExtension = (optns, done) => {
  ...
  // return
  return async.waterfall([getById, transform], done);
};
...
PredefineSchema.statics.patchByExtension = (optns, done) => {
  ...
  // return
  return async.waterfall([patch, transform], done);
};
...
PredefineSchema.statics.putByExtension = (optns, done) => {
  ...
  // return
  return async.waterfall([put, transform], done);
};
...
PredefineSchema.statics.deleteByExtension = (optns, done) => {
  ...
  // return
  return async.waterfall([del, transform], done);
};
