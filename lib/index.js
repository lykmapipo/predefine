...
const DOMAINS = env.getStringSet(
  'PREDEFINE_DOMAINS',
...
const OPTION_AUTOPOPULATE = env.getObject('PREDEFINE_AUTOPOPULATE_OPTION', {
...
const parseGivenRelations = () => {
  ...
  relations = lodash.mapValues(relations, (relation) => {
    ...
    const relationSchema = common.mergeObjects(relation, {
      type: array ? [{ type: mongooseCommon.ObjectId, ref }] : mongooseCommon.ObjectId,
      ref,
      index: true,
...
const createRelationsSchema = () => {
  ...
  const relationsSubSchema = mongooseCommon.createSubSchema(allowedRelations);
...
const transformToPredefine = (val) => {
  ...
  let predefine = common.mergeObjects({
    ...
    properties: lodash.omit(
      common.mergeObjects(data.properties, lodash.omit(data, ...knownPaths)),
      ...knownPaths
    ),
  });
      ...
      if (!lodash.isFunction(schema.statics[methodName])) {
        ...
        schema.static({
          ...
          [methodName](...args) {
            ...
            if (lodash.isArray(fakes)) {
              lodash.forEach(fakes, (fake) => {
      ...
      if (!lodash.isFunction(schema.statics[methodName])) {
        ...
        schema.static({
          ...
          [methodName](filter, projection, options, callback) {
            ...
            const actualFilter = lodash.isFunction(filter)
              ? common.mergeObjects({ namespace, bucket })
              : common.mergeObjects({ namespace, bucket }, filter);
...
const findRecursiveByNamespace = (schema) => {
  ...
  lodash.forEach(NAMESPACE_MAP, (predefine) => {
    ...
    if (common.areNotEmpty(namespace, bucket)) {
      ...
      if (!lodash.isFunction(schema.statics[parentsMethodName])) {
        ...
        schema.static({
          ...
          [parentsMethodName](criteria, callback) {
            ...
            const actualCriteria = common.mergeObjects(
              { namespace, bucket },
              criteria
            );
      ...
      if (!lodash.isFunction(schema.statics[childrenMethodName])) {
...
PredefineSchema.statics.prepareSeedCriteria = (seed) => {
  ...
  const fieldsCriteria = common.flat(
    lodash.pick(copyOfSeed, 'namespace', 'bucket', 'domain', 'strings.code', ...names)
  );
...
PredefineSchema.statics.getOneOrDefault = (criteria, done) => {
  // normalize criteria
  const { _id, namespace, bucket, domain, ...filters } = common.mergeObjects(criteria);
...
PredefineSchema.statics.findChildren = (criteria, done) => {
  ...
  const collectResults = (...updates) => {
    let collected = common.uniq([...results, ...updates]);
    collected = lodash.uniqWith(collected, mongooseCommon.areSameInstance);
    return collected;
  };
  ...
  const findKids = (conditions, next) => {
    ...
    return query.exec((error, children) => {
      ...
      if (!lodash.isEmpty(children)) {
        ...
        const parentIds = common.uniq(mongooseCommon.toObjectIds(...children));
        if (lodash.isEmpty(parentIds)) {
...
PredefineSchema.statics.findParents = (criteria, done) => {
  ...
  const findAncestors = (conditions, next) => {
    ...
    return query.exec((error, ancestors) => {
      ...
      if (!lodash.isEmpty(ancestors)) {
        results = collectResults(...ancestors);
        const ancestorIds = common.uniq(lodash.map(ancestors, 'relations.parent'));
        const parentIds = common.uniq(mongooseCommon.toObjectIds(...ancestorIds));
        if (lodash.isEmpty(parentIds)) {
          return next(null, results);
        }
